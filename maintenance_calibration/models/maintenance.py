...
class MaintenanceEquipment(models.Model):
    ...
    def _prepare_request_from_plan(self, maintenance_plan, next_maintenance_date):
        result = super(MaintenanceEquipment, self)._prepare_request_from_plan(maintenance_plan, next_maintenance_date)
        if maintenance_plan.maintenance_kind_id.calibration:
            result['maintenance_type'] = 'calibration'
            result['name'] = "%s - %s" % (self.name, maintenance_plan.maintenance_kind_id.name)
        return result
...
class MaintenanceRequest(models.Model):
    ...
    maintenance_type = fields.Selection(selection_add=[('calibration', 'Calibration')])